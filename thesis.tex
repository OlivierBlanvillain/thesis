...
The binary sizes of implicit-based implementations grows linearly with the input size, by 6 bytes per input size for smaller benchmarks (Concat \& Remove), and by 12 and 18 bytes for the larger benchmarks (Reduce \& Join).
To put that in perspective, those increments represent

concat-implicits.scala.001.scala: 16223
join-implicits.scala.001.scala: 22485
numpy-implicits.scala.001.scala: 22215
remove-implicits.scala.001.scala: 16321

type-level operation, which we implemented once using implicits (\Cref{chap:ab-using-implicits}), once using generalized singletons (\Cref{chap:generalizing-scala-s-singleton-types}), and once using match types (\Cref{chap:match-types}).


All our 4 benchmarks are type-level-only
...
\lipsum[1-2]
\Cref{fig:timingGraphsTooEarlyTooLate}
\lipsum[1-2]
