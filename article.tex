...
\subsection{Comparison to Generalized Singleton Types}

In \Cref{chap:generalizing-singleton-types}, we presented a generalization of singleton types for type-level programing.
...
The two features relate on deeper level: we can explain match types in terms of generalized singletons.
For example, the we can give an alternative definition of |Elem| match type using a generalized singleton:
%
\begin{lstlisting}
...
This updated definition of disjointness allows us to maintain the property of \Cref{lem:disjointness-subtyping-exclusivity} in the presence of empty types (disjointness/subtyping exclusivity).
\end{diff}
...
\subsection{Match Type Variance}

\begin{diff}
In \Cref{subsec:disjointness-of-variant-types} we discussed how our match type reduction algorithm handles variance annotation of higher-kinded types.
In this section, we discuss the orthogonal problem of supporting variance annotations on match type definitions.

At the time of writing, our implementation does not support variance annotations on match types (all match types are invariant).
However, it would be conceivably to lift this restriction in the future, as shown by the analysis presented in this section.
...
Given this particular definition of |M|, we can write a direct semantic definition of |M| with equations:
%
\begin{itemize}
