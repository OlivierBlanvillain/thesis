...
Scala's implicits allow us to define type-level processing operations for heterogeneous lists.
We develop the example presented the introduction by looking into the type-level remove operation on |HList|s.
Remove takes as argument an element and a list, and returns a new list with the first occurence of the element removed.
This operation should yield an error if the element is not part of the list.

An implicit-based type-level operation take the form a trait, the operation's entry point, and several implicit definitions, one for each "case" of the operation's algorithm.
Let us consider the implementation of the remove operation in more details:
...
\noindent
The |Remove| trait takes 3 type parameters, 2 inputs, |V| (the element to remove) and |Ps| (the list), and one output, |Out| (the list, with the first occurrence of the element removed).
...
The |Remove| trait is intended to be used as an implicit parameter, with constrained input types, and unconstrained output type.

If the given element is not part of the list, the corresponding implicit resolution will fail with an "implicit not found" error, which indicates the incorrect use of |Remove|.
More precisely, the implicit resolution will first iterate thought the list by repeatedly using |casetail|, until it reaches |Remove[V,HNil,Out]|, which is unsatisfiable (neither |casehead| nor |casetail| can produce an instance of |Remove| with |Ps=HNil|).
...
This method's documentation specifies 5 alternatives for the |event| argument, but this constrain is not reflected in the method's type signature.
In JavaScript, calling the |addEventListener| method with an unknown element event type is a \emph{no-op}, which can make such error particularly hard to spot.

Instead of specifying this constrain in the documentation, we can represent the alternatives in a |HList|, and use an implicit parameter of type |Remove|, to statically enforce that property:
...
In case of valid event types |E|, the compiler will syntheses an implicit evidence of type |Remove[E,EventTypes,?]|, witnessing |E|'s validity.
After implicit resolution, calls to the |addEventListener| method are expanded to insert implicit parameters in place of the second parameter list, such as in the following example:

\addEventListenerImplicitCall

\section{Implicit Priorities}

\citep{odersky2009arrays}

\section{Implicit Ambiguities}
