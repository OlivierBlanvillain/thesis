...
In particular, through extended examples, we show how to exploit Scala's implicit resolution mechanism to \emph{compute} types,  by the means of relations and constraints, in a style that resembles logic programming.

We explore the compromises involved in simulating variations on the theme of the dependent function space in an attempt to help programmers put dependent types to work.
...
We presented implicit prioritization, another tool in the implicit programmer arsenal that allows to work around implicit ambiguities.
