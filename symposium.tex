...
This \begin{diff}\paper is\end{diff} structured as follows.
...
We conclude the \begin{diff}\paper in\end{diff} \Cref{sec:conclusion}.
...
\begin{enumerate}
  ...
  \item The runtime capturing group processing component, which extracts and sanitizes the output of the \begin{diff}JVM's\end{diff} regex engine, in accordance with the previously computed type-level representation.
...
% TODO: could you explain in more detail why "the Singleton bound is necessary to guide Scala’s type inference"?
...
% TODO L196: "Furthermore, the type of those groups corresponds to the type of the tuple elements." This part is a bit hard to understand at a first reading, because only later on L236, the reader learns that matched values can only be of type String or Option String, and there's a singular/plural mismatch ("the type" is singular but applies to each of "those groups"). It might help to rephrase this sentence and already mention "String or Option String".
...
% TODO: How understandable are the compiler's error messages if the programmer makes a regex-related mistake? In particular, what's the error message if the programmer provides an incorrect number of arguments to an extractor? What's the error message in case of unbalanced parentheses in the regex? What about other forms of invalid regexes? Could you list the kinds of regex-library-related error messages to be expected?
...
% Another type-safe regex library that is not discussed in related work is Tyre: https://github.com/Drup/tyre
% TODO \citep{radanne2020tyre}
