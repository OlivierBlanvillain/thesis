...
In Scala 3, null values no longer inhabit every type: nullable types require explicit annotations of the form \dotty{A | Null} \citep{nieto2020scala}.
...
% Shape and indexing errors in ndarrays is a widely acknowledged problem~\citep{barham2019stuck, rush2019namedtensor}, and several solutions have been suggested.
...
% For instance, in a functional pearl on typed matrices~\citep{santos2020match}, the authors report that their first (simpler) attempt was deemed inconclusive due to limitations of the Haskell type inference when combining type families with pattern matching.
...
In their introductory example,~\citepauthor{weirich2011generative} show how type family reduction can lead to unsoundness in the absence of role annotations.
...
Although the situation is concerning, it might not be as bad as it seems given that soundness is a non-goal of TypeScript's type system~\citep{bierman2014typescript}.
