...
Through our formalization and implementation in the context of an industrial-strength compiler, we hope to provide a reference point for other language designers interested in adding dependent types.
...
With these goals in mind, we extended Scala's type system to lift programs and partially evaluate them as part of type-checking.
Users can manipulate dependent types either directly, using new syntactic forms in Scala's type language, or have them inferred automat
ically using an additional keyword.
This effectively allows users to execute programs involving functions and pattern matching to the type level.

We implemented our type system on top of Dotty, the next-generation compiler of Scala.
Working in the context of a production compiler we designed our implementation with backward-compatibility and interoperability in mind
.
Our system is backward-compatible in the sense that all existing programs continue to compile, but also interoperable, in that existing
code can be used from dependently-typed programs and vice versa.
As we will see, subtyping plays a key role in achieving interoperability.
...
\begin{itemize}
\item
  We begin by motivating why dependent types are desirable and how one might use them to improve type safety (\Cref{sec:motivating-example}). Our example demonstrates how to design a strongly-typed API in a functional style accessible to programmers.
\item
  We describe how we extended Scala with dependent types (\Cref{sec:extending-scala}). This practical introduction would be of interest to any Scala programmer willing to learn how to use our system, as well as programming language designers interested in adding dependent types to their language with subtyping.
\item
...
Our formalization is out of the scope of this thesis...
