...
\noindent
This method's documentation specifies 5 valid alternatives for the |event| argument, but that constrain is not reflected in the method's type signature.
...
Instead of specifying that constrain in the documentation, we can represent the valid event types in a |HList|, and use an implicit parameter of type |Remove| to statically enforce that property:
...
We have introduced an implicit-based pattern to encode type-level computation.
