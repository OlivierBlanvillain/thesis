...
This trait is intended to be used as an implicit parameter, with constrained input types, and unconstrained output type.

The remove operation is undefined if the element is not part of the list.
In that case, implicit resolution will fail with an "implicit not found" error.
More precisely, implicit resolution will iterates thought the list by repeatedly using |casetail|, until it reaches |Remove[V,HNil,Out]|, which is unsatisfiable (neither |casehead| nor |casetail| can produce an instance of |Remove| with |Ps=HNil|).
...
This method clearly specifies a fixed set of alternatives for its |event| argument, but this constrain is not reflected in the method's type signature.
In JavaScript, calling this method with unknown element event type is a \emph{no-op}, which can make errors particularly hard to spot.

Instead of specifying this constrain in the documentation, we list the alternative in a |HList| type, and use an implicit parameter of type |Remove|:
