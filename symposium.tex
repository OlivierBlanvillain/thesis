...
\regexBars{A comparison of the compilation time (left) and execution time (right) of Scala's standard regex library (Std) against our library with its code duplicated runtime (Dup) and with its implicit-based runtime (Impl) on our test suite (lower is better).}

In this section... \Cref{fig:regexBars}.
