...
\begin{enumerate}
  ...
  \item In \Cref{subsec:implicit-based-extractor-synthesis}, we show how to use implicits to perform type-directed code synthesis: we use the output of our type-level analysis as an input of implicit search to generate a type-specialized capture group sanitizer.
\end{enumerate}
...
From a design standpoint, this solution is appealing as it leads to no duplicated code or computation: we perform our capture group analysis at the type-level, once and for all, and synthesize code accordingly.
...
\regexBars{A comparison of the compilation time (left) and execution time (right) of Scala's standard regex library (Std) against our library with its code-duplicated runtime (Dup) and with its implicit-based runtime (Impl). Lower is better.}
...
We run our benchmarks on the latest version of Scala~3 (3.1.2 at the time of writing).

\paragraph{Compilation Times} Our library in its code-duplicated variant adds a total of 53~ms over the baseline, which corresponds to an average of +0.3~ms per regex.
...
When it comes to compilation times, it would be misleading to compare increments relative to the overall time, given that the compiler spends a large portion of its time outside of the type checker \citep[2.11.3]{petrashko2017design}.

\paragraph{Execution Times} At runtime, the code-duplicated variant adds 51~\textmu s over the baseline.
...
The implicit-based variant gets rid of this cost by synthesizing a type-specialized runtime (at the cost of increased compilation timed).
...
Our implicit-based implementation appears to be more prone to inlining and partial evaluation since it does not involve generic sequences, which could explain the slight grain in performance.
...
\citeauthor{spishak2012type} developed a type system to validate regular expression syntax and capturing group extraction in Java programs \citep{spishak2012type}.
...
Unlike our system, their are not concerned with null safety and does not detect optional capturing groups.
...
\citep{mainland2007s}'s quasiquote

Also this talk, from Stephanie: \citep{weirich2014examples}
dependently-typed flavor of GHC Haskell which requires no less than 20 language extensions, whereas our implementation compiles with vanilla Scala~3.
