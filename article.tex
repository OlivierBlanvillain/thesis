...
This paper presents \emph{match types}, a type-level equivalent of pattern matching.
...
Although the combination of subtyping and type-level programming has been studied extensively on the theoretical side, through the means of dependently typed systems~\cite{aspinall1994singleton, zwanenburg1999pure, stone2000deciding, courant2003strong, hutchins2010pure, yang2017unifying}, the practical side remains largely unexplored.
...
A conditional type, written \typescript{S extends T ? Tt : Tf}, reduces to \typescript{Tt} when \typescript{S} is a subtype of \typescript{T}, to \typescript{Tf} when \typescript{S} is not a subtype of \typescript{T}, and is left unreduced when types variables do not allow to draw a conclusion. Unfortunately, the algorithm used to reduce such conditional types is both \emph{unsound} and \emph{incomplete}.
...
This paper presents an alternative construct for type-level programming based on subtyping, which we call \emph{match types}.
...
\end{lstlisting}
%
The example, which we explain in detail in \Cref{sec:overview}, defines the type \dotty{Elem} by matching on the type parameter \dotty{X}.
%
...
In this paper, we explore the theoretical foundations of match types through the lens of a type system which extends \SystemFsub with pattern matching at the term and type level.
%
...
\begin{itemize}
  ...
  \item We formalize match types in the self-contained calculus \SystemFm and prove it sound, providing a theoretical basis of our implementation (\Cref{sec:formalization}). The paper is accompanied by a mechanization of \SystemFm, including proofs of progress and preservation.
...
\end{lstlisting}
%
This example defines a type \dotty{Elem} parametrized by one type parameter \dotty{X}. The right-hand side is defined in terms of a match on the type parameter -- a \emph{match type}.
...
\end{lstlisting}
%
Here we use \dotty{S =:= T} to denote type equality between the two types \dotty{S} and \dotty{T}, witnessed by mutual subtyping.
To reduce a match type, the scrutinee is compared to each pattern, one after the other, using \emph{subtyping}.
For example, although \dotty{String} is a subtype of both \dotty{String} and \dotty{Any} (the top of Scala's subtyping lattice), \dotty{Elem[String]} reduces to \dotty{Char} because the corresponding case appears first.

When the scrutinee type is a \dotty{List}, the match type \dotty{Elem} is defined recursively on the element type of the list.
Hence, in our example \dotty{Elem[List[Int]]} first reduces to the type \dotty{Elem[Int]}, and eventually to the type \dotty{Int}.
...
\end{lstlisting}
%
This definition is well-typed because the match expression in \dotty{elem}'s body has the exact same scrutinee and pattern types as \dotty{Elem[X]} (the function's return type).

Thanks to Scala's type inference, a call to the \dotty{elem} function can have a result type that \emph{depends} on a term-level parameter.
For instance, in the expression \dotty{elem(1)}, the Scala compiler infers the singleton type \dotty{X = 1} for \dotty{elem}'s type parameter.
This expression thus has type \dotty{Elem[1]}, which reduces to \dotty{Int} (via \dotty{Elem}'s third case).
Similarly, in \dotty{elem(x)}, the compiler infers the singleton type \dotty{X = x.type} and the expression has type \dotty{Elem[x.type]}, which might reduce further at the callsite depending on \dotty{x}'s type.
...
The necessity for disjointness is best illustrated with an example. Consider \dotty{Seq[Int]}, the type of integer sequences. \dotty{Elem[Seq[Int]]} does not reduce:
\begin{enumerate}
  \item \dotty{Elem}'s first case does not apply because \dotty{Seq[Int]} is not a subtype of \dotty{String}.
  \item \dotty{Elem}'s second case does not apply because \dotty{Seq[Int]} is not a subtype of \dotty{List[Int]} (lists are sequences, but not the other way around).
  \item \dotty{Elem}'s third case is \emph{not considered} because \dotty{Seq[Int]} and \dotty{List[Int]} are not disjoint.
\end{enumerate}
Therefore, the reduction algorithm gets stuck on the second case and the overall type is irreducible.
Without disjointness, \dotty{Elem[Seq[Int]]} would reduce to \dotty{Seq[Int]} (via \dotty{Elem}'s third case), which would be unsound. For example, the expression \dotty{elem[Seq[Int]](List(1,2,3))} would have type \dotty{Seq[Int]}, but evaluates to the integer \dotty{1}.
...
In this section, we formally present \SystemFm, an extension of \SystemFsub~\cite{cardelli1994extension} with pattern matching, opaque classes, and match types.
...
At the term level, a type test consists of an inheritance check against a particular class (this is also known as a typecase \cite{abadi1991dynamic}).
...
Throughout this paper, we use the abbreviated syntax $\t_s \match \{ \x_i \: \C_i \⇒ \t_i \} \otherwise \t_d$ to denote an arbitrary number of cases, that is, $\∃ \n \∈ \ℕ.~ \t_s \match \{ \x_{1} \: \C_{1} \⇒ \t_{1}; \dots; \x_n \: \C_n \⇒ \t_n \} \otherwise \t_d$.
...
% Even though \Fm is limited to type test patterns, more elaborate patterns can be expressed using type tests.
% For example, consider the following data type deconstruction pattern (left-hand side):
...
% list match {
%   case l : ::[Any] => print(l.head)
%   case Nil => ()
% }

% \noindent
% This pattern could alternatively be written as type test pattern (right-hand side), were \dotty{::[Any]} is the type of Scala's cons node.

\subsubsection*{Match Type Reduction}
The subtyping relation contains five rules for match type reduction, \SMatch1/2/3/4/5.
...
In this case, the match type is \emph{stuck}, and is assigned as is.
Stuck types can appear as the result of programming error, but can also be due to the local irreducibility of a match type.
For instance, the body of~$\f$ is typed with a stuck type, as shown in \Cref{ex:simplereduction}, but that type can later become reducible depending on type variable instantiations.
...
The first version is a pen-and-paper proof where \SystemFm is exactly as presented in \Cref{fig:FMTypingRules}.
The second version is a mechanization of the proof in Coq,
...
% There is nothing special about this, omitted since it is standard.
% An important technical detail that we omitted in \Cref{fig:FMTypingRules} concerns the well-formedness of types on contexts.
% A type $\T$ is well-formed in $\Γ$ when all the free variables in $\T$ are bound in $\Γ$.
% A context $\Γ$ is well-formed if each name is bound at most once and if each binding is to a well-formed type in the context preceding its binding.
% To improve presentation, we have omitted all well-formedness premises from our typing rules.
...
The basic structure resembles that of \SystemFsub's standard safety proof from~\cite{pierce2002types}.
...
Our proof uses a mapping from \SystemFm's types into non-empty subsets of a newly defined set $\P$.
...
\harpoon{Definition of the auxiliary relation $\⇌$, used to state inversion of subtyping (\Cref{lem:inversion-of-subtyping}).}
...
In the new subtyping rule for non-default match reduction, \BSMatch1/2, the first premise instantiates the pattern variable $\X$ to some type $\U$ such that the scrutinee type is a subtype of the $\n$th pattern:
\BSMatchInline
...
In Scala 3, null values no longer inhabit every type: nullable types require explicit annotations of the form \dotty{A | Null} \cite{nieto2020scala}.
...
% To allow easy migration from older versions, the strict treatment of nulls is still optional in Scala 3.0, enabled by the command line option \dotty{-Yexplicit-nulls}. The plan is to make strict null checking the default in the future.
...
Exhaustivity checking uses static knowledge about the class hierarchy (such as the sealed and final annotations) to check that every value in the scrutinee type is covered by the pattern clauses \cite{fengyun2016generic}.
...
On the JVM, Scala is compiled using \emph{partial erasures}, where parts of types are preserved and translated to the JVM's type systems, and other parts are erased \cite{schinz2005compiling}.
...
This mechanism is completely standard and already used in other programing languages with unbounded recursion at the type level \cite{eisenberg2014closed, vilhelm2015dependently, eisenberg2016dependent}.
...
Pattern matching in Scala supports many sorts of patterns~\cite{emir2007matching}, most of which do not have a match type counterpart.
...
To this end, we outline the type-level implementation of a library for multidimensional arrays which mimics the NumPy API~\cite{harris2020numpy}.
...
Shape and indexing errors in ndarrays is a widely acknowledged problem~\cite{barham2019stuck, rush2019namedtensor}, and several solutions have already been proposed, notably in the form of libraries that rely on type-level programming~\cite{chen2017typesafe, huang2017hasktorch}.
...
% Shape and indexing errors in ndarrays is a widely acknowledged problem~\cite{barham2019stuck, rush2019namedtensor}, and several solutions have been suggested.
% The Dex language~\cite{maclaurin2019dex} introduces an experimental language with a Hindley--Milner-based type system with types for index sets, but cannot express variable-rank ndarrays and lacks the type-level arithmetic needed for typing reshapes.
% Nexus~\cite{chen2017typesafe} represents variable-rank ndarrays as HLists of dimension labels, and uses Scala's implicit resolution to express type-level transformations on shapes.
...
% The implementation in this case study is inspired by the HaskTorch Haskell library~\cite{huang2017hasktorch}, which also represents shapes using HLists, and makes direct use of type-level computation to compute shape transformations.
...
Scala supports singleton types, which are types inhabited by a single value~\cite{leontiev2014sip}.
...
We represent the shape of an ndarray at the type level using a heterogeneous type list, or HList for short~\cite{kiselyov2004strongly}.
...
This unification is also commonly used in the presence of subtyping~\cite{zwanenburg1999pure, hutchins2010pure, yang2017unifying}.
...
Singleton types provide an interesting middle ground between unified and separate syntax and have also been studied in conjunction with dependent types and subtyping~\cite{aspinall1994singleton, stone2000deciding, courant2003strong}.
...
Dependent Object Types (DOT) are, to this day, the most significant effort in formalizing Scala's type system~\cite{essence2016amin}.
DOT does not directly support any form of type-level computation.
We considered using DOT as a starting point for our work, however, despite the recent effort to simplify DOT's soundness proof~\cite{rapoport2017simple, giarrusso2020soundness}, extending DOT remains too big of a challenge to concisely describe language extensions.
...
In their work on intensional type analysis~\cite{harper1995compiling}, Harper and Morrisett introduce the $\λ^{ML}_i$ calculus that supports structural analysis of types.
...
Haskell's type families allow programmers to define type-level functions using pattern matching~\cite{chakravarty2005associated, schrijvers2008open}.
...
Although the formalization presented in~\cite{eisenberg2014closed} does not cover non-terminating families, the paper discusses a soundness problem caused by non-termination.
...
% A significant difference between type-checking type families and match types is that type families are entirely handled during type inference, while match types reduce during subtyping.
% Influencing Haskell's global type inference is notoriously difficult, which can, unfortunately, hinder the usability of type families.
% For instance, in a functional pearl on typed matrices~\cite{santos2020match}, the authors report that their first (simpler) attempt was deemed inconclusive due to limitations of the Haskell type inference when combining type families with pattern matching.
...
The TypeScript language specification briefly describes the algorithm used to reduce conditional types in the presence of type variables~\cite{microsoft2020typescript}.
...
Although the situation is concerning, it might not be as bad as it seems given that soundness is a non-goal of TypeScript's type system~\cite{bierman2014typescript}.
...
\section{Conclusion}
In this paper, we introduced \emph{match types}, a lightweight mechanism for type-level programming that integrates seamlessly in subtyping-based programming languages.
