...
We develop the example presented in the introduction by looking into the type-level remove operation on |HList|s.
...
More precisely, the implicit search first iterates though the list by repeatedly using |casetail| until it reaches the end of the list, at which point it fails to find an implicit value of type |Remove[V,HNil,Out]| (neither |casehead| nor |casetail| produce an instantiation of |Remove| with |Ps=HNil|).
...
In the next section, we discuss ... % implicit ambiguities Scala's priority system for implicits.
...
Implicit-based programs sometimes rely on ambiguities and priorities of implicit definition, as we will see in this section thought a series of example.

\subsection{Implicit Ambiguities}

The Scala compiler rejects programs with \emph{ambiguous} implicit definition.
For example, if we write two identically-typed implicit definitions in the same scope, the compiler will consider them ambiguous and report an error:

\implicitAmbiguity

While ambiguities typically indicate programing error, they can also be used purposefully to implement the error case of a type-level program.
For example, consider the |NotIn| operation on |HList| that

they can also be can also
...
considers definitions by \emph{scope}, the order of definitions is a given scope has no semantic significance.
...
While details of the language specification are outside of the scope of this disertations, type-level \citep{odersky2006scala} \citep{odersky2009arrays}
