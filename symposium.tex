...
In this \thesisOnly{chapter}\paperOnly{paper}, we propose a new design for Scala's regular expression library which provides a type-safe and null-safe mechanism for capturing group extraction.
...
\Cref{sec:background}
  \item
\Cref{sec:architecture},~\ref{sec:type-level} and~\ref{sec:term-level}
  \item
\Cref{sec:evaluation}
  \item
\Cref{sec:related}
...
\subection{Match Types}

In this section, we give a brief introduction to match types by inspecting the following example:
...
\subection{The Generic View of Tuples}
...
In this \thesisOnly{chapter}\paperOnly{paper}, we introduced a new design for type-safe regular expressions in Scala.
