...
As an example usage of |NotIn|, ...:b
...
The compiler will synthesize an implicit parameter of type |NotGiven[T]|, if and only if there is no implicit of type |T| in scope.
We can use a "negative" implicit evidence to simplify the definition of |NotIn| by removing the |ambiguous| definitions and changing |casecons| to the following:
...
While details of the language specification are outside of the scope of this dissertation, type-level \citep{odersky2006scala} \citep{odersky2009arrays}
