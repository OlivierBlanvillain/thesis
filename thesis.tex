...
Early attempts of match type reduction are trade-off: they can be either an small overhead, if the reduction fails, a no-op, if the reduction succeeds but the result is used exactly once, or an optimization, if the reduction succeeds and the result is used more than once.
Note that this trade-off is laziness does not
