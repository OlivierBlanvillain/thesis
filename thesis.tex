...
As we will see in \Cref{chap:performance-based-evaluation}, those techniques come at a high cost in terms of compilation time, which hinders their usability on a large scale.
...
This chapter is based on~\citep{blanvillain2022type}, which was written in collaboration with Jonathan Brachthäuser, Maxime Kjaer, and Martin Odersky, and published in POPL'22. \Cref{sec:case-study-shape-safe-num-py} is based on Maxime's semester project entitled "Shape-safe TensorFlow in Dotty", where he designed a strongly typed TensorFlow interface in Scala, which checks tensor shapes at compile-time in order to prevent runtime errors in machine learning models. Maxime also contributed to the Scala~3 compiler by adding support for arithmetic computations at the type level. Jonathan single-handedly mechanized \SystemFm's type safety proof, and provided invaluable help and guidance with redaction. Martin and I collaborated on the implementation of match types in the Scala 3 compiler, which is now actively used by the Scala community.
...
We evaluate the performance of the various type-level programming techniques presented in this dissertation by comparing them on a set of 4 benchmarks.
...
These benchmarks are inspired by code examples used throughout this dissertation. They all follow a similar pattern of a single function call whose result type is computed at the type level. We code generated variations of each benchmark with increasing input size, from 1 to 256, in +8 increments.
...
We briefly describe the 4 benchmarks used in this chapter:

\begin{enumerate}
  \item \textbf{concat}\quad
  Concatenates a heterogeneous list with itself, resulting in a list twice as long.
...
\Cref{fig:timingGraphsZoomedOut} compares the compilation time of implicits, generalized singletons and match types on our benchmark sets.
...
These results insinuate that implicits are ill-suited for large scale, type-level computations: spending tens of seconds to compile a single operation is clearly out of the question for any practical use case.

\Cref{fig:timingGraphsZoomedIn} shows those same results rescaled, with implicits

We obtained the data points of this figure by running


\timingGraphsZoomedOut{A comparison of the compilation time of implicits, generalized singletons and match types for our benchmark set (lower is better).}

\timingGraphsZoomedIn{A comparison of the compilation time of generalized singletons and match types for our benchmark set (lower is better).}

\clearpage
\section{Binary size}

\Cref{fig:bytecodeSizeTable}
\bytecodeSizeTable{Additional JVM Bytecode generated\\per benchmark size increment (in Bytes)}

\clearpage
\section{The Timing of Match Type Reduction}
\timingGraphsTooEarlyTooLate{Caption}
