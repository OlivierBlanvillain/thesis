...
Each early attempt of early match type reduction is thus a trade-off: it can be either an overhead, if the reduction fails, a no-op, if the reduction succeeds but the result is used only once, or an optimization, if the reduction succeeds and the result is used more than once.
