...
Doing so is always be correct, since the types before and after reduction are mutually subtypes of each other.
Each early attempt of match type reduction is trade-off: it can either be a small overhead, if the reduction fails, a no-op, if the reduction succeeds but the result is used exactly once, or an optimization, if the reduction succeeds and the result is used more than once.

The following example illustrates the benefits of early reduction attempts:
...
Note that laziness does not this trade-off is laziness does not

Laziness does not solve that problem straight out because caches for match type reductions are context-dependent.

In \Cref{fig:timingGraphsTooEarlyTooLate}, we compare the compilation times of the match type Reduce benchmark with two different match type reduction strategies.
The first strategy (S1), attempts to reduce match types twice, once right after creation, and once during subtyping. The second strategy (S2) extends S1 with additional reduction attempts before type substitution.
The first strategy fails...

\timingGraphsTooEarlyTooLate{A comparison of the compilation time of the match type Reduce benchmark with two different match type reduction strategies. S1 reduces match types after creation and during subtyping. S2 extends S1 by also reducing before type substitution.}
